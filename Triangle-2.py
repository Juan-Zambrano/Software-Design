...
#  Partner's Name: Audrey Mcnay 

#  Partner's UT EID: alm5735
...
def exhaustive_helper(grid, i, idx, sol, sum_n):
  if(idx == len(grid)):
    sol.append(sum_n)   
  else:
    sum_n += grid[idx][i]
    exhaustive_helper(grid,i ,idx +1,sol,sum_n) 
    exhaustive_helper(grid,i+1,idx+1,sol,sum_n)
  return sol

# returns the greatest path sum using exhaustive search
def exhaustive_search (grid):
  return exhaustive_helper(grid,0,0,[],0)
...
def rec_helper (grid, row, index):
  #base case
  if row == (len(grid)-1):
    return grid[row][index]
  #recursive case
  else:
    return grid[row][index] + max((rec_helper(grid, row+1, index)), rec_helper(grid, row+1, index+1))
...
def main ():
  ...
  ti = time.time()
  exhaust = max(exhaustive_search(grid))
  tf = time.time()
  del_t = round((tf - ti),3)
